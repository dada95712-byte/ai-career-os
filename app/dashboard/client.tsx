...
export function DashboardClient({ name }: { name: string }) {
  ...
  return (
    <div className="min-h-screen bg-cream-100 p-6 lg:p-8 space-y-6">
      ...
      {/* ── Row 1: Greeting + Score (left) | Today's Task (right) ── */}
      ...
      {/* ── Career Coach (full width) ─────────────────────────────── */}
      <div className="rounded-2xl bg-sage-50 border-l-4 border-l-sage-400 px-5 py-4 flex items-center gap-4"
        style={{ boxShadow: 'var(--shadow-warm-sm)' }}>
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-sage-100 text-base">
          🌿
        </div>
        <p className="flex-1 text-sm text-ink-600 leading-relaxed">
          你的履歷目前缺少量化成就數據，加入 2–3 個具體數字可讓 ATS 分數提升約 30 分。
        </p>
        <Link href="/career-profile" className="shrink-0">
          <Button variant="outline" size="xs">優化履歷 →</Button>
        </Link>
      </div>
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-2 pt-1">
            ...
            <div className="pt-2 text-right">
              <Link href="/career-match" className="text-xs text-terra-500 hover:text-terra-700 transition-colors font-medium">
                查看全部 →
              </Link>
            </div>
          </CardContent>
